import pygame
import os
# esto es un modo de imagen para ponerlo desde otros paquetes
...
class Piece:
    ...
    def draw(self, win, color):
        ...
        '''if self.selected y self.color == color: # Elimina falso para dibujar puntos
             mueve = self.move_list

             para mudanzas:
                 x = 33 + round (self.startX + (move [0] * self.rect [2] / 8))
                 y = 33 + round (self.startY + (move [1] * self.rect [3] / 8))
                 pygame.draw.circle (ganar, (255, 0, 0), (x, y), 10)'''
